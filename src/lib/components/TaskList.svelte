...
<section class="flex flex-col h-full bg-base-100 border border-base-100">
  ...
  {:else}
    <ul class="flex flex-col divide-y divide-base-300 bg-base-100">
      {#each tasks as task}
        <li class="group relative flex items-center px-3 h-9 cursor-pointer select-none hover:bg-blue-50/80 focus-within:bg-blue-50/80 transition-all text-xs font-mono tracking-tight" aria-label={task.name}>
          ...
          <input type="checkbox" class="checkbox checkbox-xs mr-2" on:change={() => onToggle(task.id)} />
          ...
          <span class="flex flex-row gap-1 ml-2">
            <button class="btn btn-xs btn-ghost px-1" aria-label="Edit task" on:click={() => onEdit(task.id)}></button>
            <button class="btn btn-xs btn-ghost px-1" aria-label="Delete task" on:click={() => onDelete(task.id)}></button>
          </span>
        </li>
      {/each}
    </ul>
